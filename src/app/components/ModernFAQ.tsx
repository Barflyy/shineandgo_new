    ...
    <section className="relative py-12 md:py-16 lg:py-24 bg-white overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 md:mb-12 lg:mb-16">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-brand-50 text-brand-700 text-xs sm:text-sm font-semibold mb-4 sm:mb-6">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display font-bold text-dark-900 mb-4 md:mb-6 px-4">
            ...
            <span className="block mt-1 text-base sm:text-lg md:text-xl lg:text-2xl text-dark-600">Nous avons les réponses</span>
          </h2>
        </div>
        ...
        <div className="max-w-3xl mx-auto space-y-3 md:space-y-4">
          {faqs.map((faq, idx) => (
            <div
              key={idx}
              className="group rounded-xl sm:rounded-2xl border-2 border-dark-200 bg-white overflow-hidden transition-all duration-300 hover:border-brand-300 hover:shadow-lg"
            >
              <button
                onClick={() => setOpenIndex(openIndex === idx ? null : idx)}
                className="w-full flex items-center justify-between gap-3 sm:gap-4 p-4 sm:p-5 md:p-6 text-left transition-colors touch-manipulation"
              >
                <span className="text-sm sm:text-base md:text-lg font-semibold text-dark-900 group-hover:text-brand-600 transition-colors pr-1 sm:pr-2">
                  {faq.question}
                </span>
                <ChevronDown
                  className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 flex-shrink-0 text-dark-600 transition-transform duration-300 ${
              ...
              >
                <div className="overflow-hidden">
                  <div className="px-4 sm:px-5 md:px-6 pb-4 sm:pb-5 md:pb-6 text-xs sm:text-sm md:text-base text-dark-600 leading-relaxed">
                    {faq.answer}
                  </div>
                </div>
              </div>
        ...
        <div className="mt-8 md:mt-12 lg:mt-16 text-center px-4">
          <p className="text-dark-600 text-sm sm:text-base mb-4 sm:mb-6">Vous ne trouvez pas la réponse à votre question ?</p>
          <a
            href="https://wa.me/32472303701"
            className="inline-flex items-center gap-1.5 sm:gap-2 px-6 sm:px-8 py-3 sm:py-4 rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 text-white font-semibold text-sm sm:text-base shadow-glow hover:shadow-glow-lg transition-all duration-300 hover:scale-105 touch-manipulation"
          >
