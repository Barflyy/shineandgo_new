...
function HeroSection() {
    const handleBooking = () => { window.open('https://wa.me/32472303701?text=Bonjour Nathan ! Je souhaite réserver un lavage voiture à Saint-Nicolas. 🚗', '_blank'); };
                ...
                <div className="max-w-3xl mx-auto">
                    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex justify-center mb-6">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200/80 shadow-sm">
                            ...
                            <span className="text-xs font-semibold text-slate-700">Déplacement gratuit à Saint-Nicolas</span>
                        </div>
                    </motion.div>
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }} className="text-center mb-6">
                        <h1 className="font-display text-[2.5rem] leading-[1.1] sm:text-5xl md:text-6xl font-extrabold tracking-tight text-slate-900 mb-4">
                            ...
                            <span className="relative inline-block">
                                <span className="relative z-10 bg-gradient-to-r from-sky-500 to-emerald-500 bg-clip-text text-transparent">à Saint-Nicolas</span>
                        ...
                        <p className="text-lg sm:text-xl text-slate-600 max-w-xl mx-auto leading-relaxed">
                            Valeting professionnel à domicile dans la <strong className="text-slate-900">banlieue liégeoise</strong>. Parking difficile ? Rues étroites ? On s'en charge, vous restez chez vous.
                        </p>
                    </motion.div>
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-col gap-3 mb-8">
                        ...
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleBooking} className="w-full sm:w-auto sm:mx-auto group relative overflow-hidden px-8 py-5 bg-slate-900 text-white font-bold text-lg rounded-2xl shadow-2xl shadow-slate-900/25 transition-all duration-300">
                            <span className="relative z-10 flex items-center justify-center gap-3"><CalendarCheck className="w-5 h-5" /> Réserver à Saint-Nicolas <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></span>
...
function BenefitsSection() {
    const benefits = [
        { icon: Car, title: "Fini le parking galère", description: "Rues étroites à Saint-Nicolas ? On vient directement dans votre allée ou parking résidentiel.", color: "sky" },
        { icon: Droplets, title: "Idéal navetteurs", description: "Votre voiture brille pendant que vous êtes au travail. Retour impeccable garanti.", color: "emerald" },
        { icon: Shield, title: "Pollution urbaine traitée", description: "Poussière de frein, micro-particules, suie... On élimine les salissures de ville.", color: "violet" },
    ];
        ...
        <section className="py-16 sm:py-20 bg-white">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">Pourquoi choisir Shine&Go à Saint-Nicolas ?</h2>
                </motion.div>
        ...
        <section id="services" className="py-16 sm:py-24 bg-gradient-to-b from-slate-50 to-white">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    ...
                    <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-3">Tarifs lavage voiture à Saint-Nicolas</h2>
                    <p className="text-slate-600 max-w-md mx-auto">Tarifs transparents, déplacement gratuit à Saint-Nicolas et environs.</p>
                </motion.div>
...
function TestimonialsSection() {
    const testimonials = [
        { name: 'Karim B.', location: 'Saint-Nicolas', text: 'Impossible de trouver une place pour aller à la station. Ils sont venus dans ma petite rue, problème résolu !', avatar: '👨' },
        { name: 'Nathalie M.', location: 'Montegnée', text: 'Je rentre du boulot à 18h, ma voiture m\'attendait nickel sur le parking de la résidence. Génial !', avatar: '👩' },
        { name: 'David L.', location: 'Tilleur', text: 'La pollution et la suie avaient terni ma carrosserie. Elle brille comme neuve maintenant.', avatar: '👨' },
    ];
        ...
        <section className="py-16 sm:py-24 bg-white overflow-hidden">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    ...
                    <p className="text-slate-600">Ce que nos clients disent de nous à Saint-Nicolas</p>
                </motion.div>
...
function FAQSection() {
    const faqs = [
        { question: "Ma rue est très étroite à Saint-Nicolas, c'est possible ?", answer: "Aucun souci ! On travaille dans les petites rues et parkings résidentiels de Saint-Nicolas. On s'adapte à votre espace." },
        { question: "Je n'ai pas de parking privé, comment ça se passe ?", answer: "On peut travailler sur votre place de stationnement en voirie si nécessaire. L'essentiel c'est un accès à l'eau à proximité." },
        { question: "Traitez-vous la pollution urbaine ?", answer: "Oui ! Suie, poussière de frein, micro-particules... On utilise des produits décontaminants professionnels Koch-Chemie." },
        { question: "Pouvez-vous venir pendant que je suis au travail ?", answer: "C'est notre spécialité ! Laissez-nous les clés le matin, votre voiture sera prête à votre retour." }
    ];
...
function CTASection() {
    const handleBooking = () => { window.open('https://wa.me/32472303701?text=Bonjour Nathan ! Je souhaite réserver un lavage voiture à Saint-Nicolas. 🚗', '_blank'); };
    const zones = ['Liège', 'Ans', 'Seraing', 'Herstal', 'Grâce-Hollogne'];
        ...
        <section className="relative py-16 sm:py-24 overflow-hidden">
            ...
            <div className="relative max-w-4xl mx-auto px-5 text-center">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                    ...
                    <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">Prêt pour une voiture<br className="sm:hidden" /> comme neuve ?</h2>
                    ...
                    <div className="pt-8 border-t border-white/10">
                        <p className="text-sm text-white/50 mb-4 flex items-center justify-center gap-2"><MapPin className="w-4 h-4" /> Zones proches</p>
                        <div className="flex flex-wrap justify-center gap-2">{zones.map((zone) => (<Link key={zone} href={`/lavage-voiture-${zone.toLowerCase().replace(/é/g, 'e').replace(/-/g, '-')}`} className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm text-white/70 hover:text-white transition-all">{zone}</Link>))}</div>
                    </div>
                </motion.div>
            </div>
        </section>
...
export default function SaintNicolasClient() {
    return (
