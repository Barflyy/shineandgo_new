...
function HeroSection() {
    const handleBooking = () => {
        window.open('https://wa.me/32472303701?text=Bonjour Nathan ! Je souhaite réserver un lavage voiture à Liège. 🚗', '_blank');
    };
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-slate-200/80 shadow-sm">
                            ...
                            <span className="text-xs font-semibold text-slate-700">Déplacement gratuit à Liège</span>
                        </div>
                    </motion.div>
                    ...
                    >
                        ...
                        <p className="text-lg sm:text-xl text-slate-600 max-w-xl mx-auto leading-relaxed">
                            Valeting premium dans la <strong className="text-slate-900">Cité Ardente</strong>. Parking galère en ville ? Pollution urbaine ? On vient chez vous, problème résolu.
                        </p>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="flex items-center justify-center gap-4 text-sm text-slate-500">
                            <a href="https://wa.me/32472303701" className="flex items-center gap-1.5 hover:text-green-600 transition-colors">
                                <MessageCircle className="w-4 h-4" /> WhatsApp
                            </a>
                            <span className="w-1 h-1 bg-slate-300 rounded-full" />
                            <a href="tel:+32472303701" className="flex items-center gap-1.5 hover:text-sky-600 transition-colors">
...
function BenefitsSection() {
    const benefits = [
        { icon: Car, title: "Fini les files d'attente", description: "En ville, les stations sont bondées. On vient chez vous, zéro stress, zéro perte de temps.", color: "sky" },
        { icon: Droplets, title: "Pollution urbaine traitée", description: "Particules fines, suie des bus, poussière de frein... On décontamine votre carrosserie.", color: "emerald" },
        { icon: Shield, title: "Parking Guillemins ? OK", description: "Même sur les parkings compliqués du centre-ville. On s'adapte à votre espace.", color: "violet" },
    ];
        ...
        <section className="py-16 sm:py-20 bg-white">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">Pourquoi choisir Shine&Go à Liège ?</h2>
                </motion.div>
        ...
        <section id="services" className="py-16 sm:py-24 bg-gradient-to-b from-slate-50 to-white">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    ...
                    <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 tracking-tight mb-3">Tarifs lavage voiture à Liège</h2>
                    <p className="text-slate-600 max-w-md mx-auto">Tarifs transparents, déplacement gratuit à Liège et environs.</p>
                </motion.div>
                                ...
                                <div className={`h-full p-6 sm:p-7 rounded-2xl bg-white border-2 transition-all duration-300 ${service.popular ? 'border-emerald-200 shadow-xl shadow-emerald-500/10' : 'border-slate-100 hover:border-slate-200 hover:shadow-xl'}`}>
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="text-right">
                                            <div className="flex items-baseline gap-0.5"><span className="text-3xl font-extrabold text-slate-900">{service.price}</span><span className="text-lg font-bold text-slate-400">€</span></div>
                                            <span className={`text-xs font-medium ${service.popular ? 'text-emerald-600' : 'text-slate-500'}`}>{service.priceNote}</span>
                                        </div>
                                    </div>
                                    ...
                                    <ul className="space-y-2.5 mb-5">
                                        {service.features.map((feature) => (<li key={feature} className="flex items-center gap-2.5 text-sm text-slate-700"><Check className={`w-4 h-4 flex-shrink-0 ${service.popular ? 'text-emerald-500' : 'text-sky-500'}`} /><span>{feature}</span></li>))}
                                    </ul>
...
function TestimonialsSection() {
    const testimonials = [
        { name: 'Marc D.', location: 'Guillemins', text: 'Impossible de trouver une station libre en ville. Ils sont venus sur mon parking, problème résolu !', avatar: '👨' },
        { name: 'Isabelle K.', location: 'Outremeuse', text: 'La pollution du centre avait terni ma peinture. Décontamination + cire = résultat spectaculaire.', avatar: '👩' },
        { name: 'Thomas V.', location: 'Sainte-Walburge', text: 'Entre les embouteillages et le travail, zéro temps pour laver ma voiture. Maintenant c\'est réglé !', avatar: '👨' },
    ];
        ...
        <section className="py-16 sm:py-24 bg-white overflow-hidden">
            <div className="max-w-6xl mx-auto px-5">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
                    ...
                    <p className="text-slate-600">Ce que nos clients disent de nous à Liège</p>
                </motion.div>
                        ...
                        <motion.div key={testimonial.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="flex-shrink-0 w-[85vw] sm:w-auto snap-center">
                            <div className="h-full p-6 rounded-2xl bg-gradient-to-br from-slate-50 to-white border border-slate-100">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-lg">{testimonial.avatar}</div>
                                    <div><p className="font-semibold text-slate-900 text-sm">{testimonial.name}</p><p className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="w-3 h-3" />{testimonial.location}</p></div>
                                </div>
                            </div>
                        </motion.div>
                    ))}
...
function FAQSection() {
    const faqs = [
        { question: "Je n'ai qu'une place en voirie dans le centre, c'est possible ?", answer: "Oui ! On travaille régulièrement sur des places de parking en voirie à Liège. L'essentiel c'est un accès eau à proximité." },
        { question: "Traitez-vous la pollution urbaine sur la carrosserie ?", answer: "C'est notre spécialité en ville ! Particules fines, suie des bus, poussière de frein... On décontamine et protège." },
        { question: "Pouvez-vous venir sur le parking de mon bureau aux Guillemins ?", answer: "Bien sûr ! On intervient sur les parkings d'entreprise, résidences et copropriétés de tout Liège." },
        { question: "J'ai peu de temps entre deux rendez-vous, c'est faisable ?", answer: "Un lavage complet prend 90 min. On peut aussi faire un extérieur express en 45 min si vous êtes pressé." }
    ];
                    ...
                    {faqs.map((faq, index) => (
                        <motion.details key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden">
                            <summary className="flex items-center justify-between p-5 cursor-pointer list-none font-semibold text-slate-900 hover:bg-slate-50 transition-colors">
                                ...
                                <span className="ml-4 flex-shrink-0 w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center group-open:rotate-45 transition-transform"><span className="text-slate-500 text-lg leading-none">+</span></span>
                            </summary>
...
function CTASection() {
    const handleBooking = () => { window.open('https://wa.me/32472303701?text=Bonjour Nathan ! Je souhaite réserver un lavage voiture à Liège. 🚗', '_blank'); };
    const zones = ['Herve', 'Battice', 'Verviers', 'Spa', 'Seraing', 'Ans', 'Theux'];
        ...
        <section className="relative py-16 sm:py-24 overflow-hidden">
            ...
            <div className="relative max-w-4xl mx-auto px-5 text-center">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                    ...
                    <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">Prêt pour une voiture<br className="sm:hidden" /> comme neuve à Liège ?</h2>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mb-10">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleBooking} className="group px-8 py-4 bg-white text-slate-900 font-bold text-lg rounded-xl shadow-2xl hover:shadow-white/20 transition-all flex items-center justify-center gap-3">
                            <CalendarCheck className="w-5 h-5" /> Réserver à Liège <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                        </motion.button>
                        <motion.a href="tel:+32472303701" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-bold text-lg rounded-xl border border-white/20 hover:bg-white/20 transition-all flex items-center justify-center gap-3">
                            <Phone className="w-5 h-5" /> 0472 30 37 01
                        </motion.a>
                    </div>
                    <div className="pt-8 border-t border-white/10">
                        <p className="text-sm text-white/50 mb-4 flex items-center justify-center gap-2"><MapPin className="w-4 h-4" /> Zones proches de Liège</p>
                        <div className="flex flex-wrap justify-center gap-2">
                            {zones.map((zone) => (<Link key={zone} href={`/lavage-voiture-${zone.toLowerCase().replace(/é/g, 'e')}`} className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm text-white/70 hover:text-white transition-all">{zone}</Link>))}
                        </div>
                    </div>
                </motion.div>
            </div>
...
export default function LiegeClient() {
    return (
